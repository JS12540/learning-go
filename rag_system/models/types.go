...
Go struct fields must be:

Capitalized → to be exported

But JSON usually expects lowercase keys

So we use tags to map:

Go Field Name → JSON Key Name
...
If this field is empty → don’t include it in JSON.
...
Go reads struct tags using reflection.

It checks:

json:"fieldname,omitempty"

And uses that metadata while encoding.
*/
